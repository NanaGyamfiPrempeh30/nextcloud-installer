...
| Platform | Status |
|---|---|
| Ubuntu 24.04 LTS | Tested and verified — full install, idempotent re-run, DB-unreachable failure path (EXIT_DATABASE=40), full reset-reinstall cycle, and stale config.php recovery (REQ-24) all proven |
...
Not yet exercised by direct test: unsupported-OS exit (REQ-1), not-root exit (REQ-2), optional app installation (REQ-13), and the Let's Encrypt real-domain flow (REQ-15 — only self-signed has been exercised). The PHP repository fallback path (REQ-4) was exercised on Ubuntu 22.04 (ondrej/php PPA added, PHP 8.4 installed from scratch on a clean distro). Stale config.php recovery (REQ-24) was exercised on both Ubuntu 24.04 and 22.04 — a partial config.php left by a DB-fail run was detected and cleared, allowing maintenance:install to proceed cleanly on the retry.
