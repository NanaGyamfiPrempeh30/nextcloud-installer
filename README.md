...
| Platform | Status |
|---|---|
| Ubuntu 24.04 LTS | Tested and verified — full install, idempotent re-run, DB-unreachable failure path (EXIT_DATABASE=40), full reset-reinstall cycle, stale config.php recovery (REQ-24), optional app install/enable (REQ-13), and ONLYOFFICE Document Server integration (REQ-26) all proven |
| Ubuntu 22.04 LTS | PHP PPA fallback (REQ-4 — ondrej/php added, PHP 8.4 installed from scratch) and stale config.php recovery (REQ-24) exercised; full end-to-end install not yet proven |
...
Not yet exercised by direct test: unsupported-OS exit (REQ-1), not-root exit (REQ-2), and the Let's Encrypt real-domain flow (REQ-15 — only self-signed has been exercised). The PHP repository fallback path (REQ-4) was exercised on Ubuntu 22.04 (ondrej/php PPA added, PHP 8.4 installed from scratch on a clean distro). Stale config.php recovery (REQ-24) was exercised on both Ubuntu 24.04 and 22.04 — a partial config.php left by a DB-fail run was detected and cleared, allowing maintenance:install to proceed cleanly on the retry. Optional app management (REQ-13) was exercised on Ubuntu 24.04 across all four paths: fresh install (app:install from app store), idempotent re-run (already-enabled skip), new app added to an already-installed instance, and single bad app ID (partial-failure handling, exit 50). ONLYOFFICE Document Server integration (REQ-26) was exercised on Ubuntu 24.04: fresh install against an unreachable server (connector app installed, URL configured, connectivity check warned with raw cURL error, exit 0), idempotent re-run (app skip, URL re-set), and no-flag regression (zero [ONLYOFFICE] lines).
